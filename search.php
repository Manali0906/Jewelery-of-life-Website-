	...
	<div style="padding: 30px 120px; font-size: 25px; margin: 0 auto; display: table; width: 98%;">
		...
		<?php 
			if (isset($_GET['keywords']) && $_GET['keywords'] != ""){
				...
				if(substr($search_value, $len-1,$len)=="s"){
					$search_value=substr($search_value,0,$len-1);
				}
				$getposts = mysqli_query($link,"SELECT * FROM products WHERE item like '%$search_value%' OR pname like '%$search_value%' ORDER BY id DESC") or die(mysql_error());
